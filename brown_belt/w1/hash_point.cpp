...
struct Hasher {
    size_t operator()(const Point3D& val) const {
        const static size_t p = 9999991lu;

        return p * p * coord_hasher(val.x) + \
                p * coord_hasher(val.y) + \
                coord_hasher(val.z);
    }
    static const hash<CoordType> coord_hasher;
};

const hash<CoordType> Hasher::coord_hasher {};
