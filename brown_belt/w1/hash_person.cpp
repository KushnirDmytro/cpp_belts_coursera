...
struct AddressHasher {
    size_t operator()(const Address& addr) const {
        return
        p * p * str_hasher(addr.street) +
        p * str_hasher(addr.city) +
        int_hasher(addr.building);
    }

private:
    static const size_t p = 99'991lu;
    static const hash<string> str_hasher;
    static const hash<int> int_hasher;
};
const hash<string> AddressHasher::str_hasher{};
const hash<int> AddressHasher::int_hasher{};


struct PersonHasher {
    size_t operator() (const Person& per) const{
        return
        p * p * p * h_addr(per.address) +
        p * p * h_d(per.weight) +
        p * h_i(per.height) +
        h_s(per.name);
    }
private:
    static const size_t p = 99'991lu;
    static const hash<string> h_s;
    static const hash<int> h_i;
    static const hash<double> h_d;
    static const AddressHasher h_addr;
};
const hash<string> PersonHasher::h_s{};
const hash<int> PersonHasher::h_i{};
const hash<double> PersonHasher::h_d{};
const AddressHasher PersonHasher::h_addr{};
