...
Node* LeftmostAncesster(Node* node){
    if (node->left == nullptr)
        return node;
    else
        return LeftmostAncesster(node->left);
}

Node* first_larger_parent(Node* me, int val){
    if (me->parent)
        if (me->parent->value > val)
            return me->parent;
        else
            return first_larger_parent(me->parent, val);
    else
        return nullptr;
}
...
Node* Next(Node* me) {
    if (me->right)
        return LeftmostAncesster(me->right);
    else
        return first_larger_parent(me, me->value);
}
