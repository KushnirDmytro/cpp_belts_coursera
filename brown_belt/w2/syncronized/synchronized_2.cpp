...
public:
    ...
    struct Access {
    public:
        Access(ValT &value, mutex &mt);
        ValT &ref_to_value;
    private:
        optional<lock_guard<mutex>> g_;
    };
  ...
  Access<T> GetAccess(){
      return Access<T>(value, mt);
  }

  Access<const T> GetAccess() const {
      return Access<const T>(value, mt);
  }
private:
  T value;
  mutable mutex mt;
...
template<typename T>
template<typename ValT>
Synchronized<T>::Access<ValT>::Access(ValT &value, mutex &mt) : ref_to_value{value}, g_(mt)
{
}

//template<typename T>
//template<typename ValT>
//Synchronized<T>::Access<ValT>::Access(const T &value, mutex &mt) : ref_to_value{value}
//{
//}
