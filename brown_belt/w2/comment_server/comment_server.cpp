...
ostream& operator<<(ostream& os, const HttpCode& code){
    os << static_cast<std::underlying_type<HttpCode>::type>(code);
    return os;
}
...
public:
    ...
    friend ostream& operator << (ostream& output, const HttpResponse& resp){
        static const string respond_prefix = "HTTP/1.1";
        static unordered_map<HttpCode, string> comments{
                {HttpCode ::Ok, "OK"},
                {HttpCode ::Found, "Found"},
                {HttpCode ::NotFound, "Not found"},
        };
        output << respond_prefix << ' ' << resp.code_ << ' ' << comments[resp.code_] << '\n';

        for (const auto &header_pair: resp.headers_){
            output << header_pair.first << ": " << header_pair.second << '\n';
        }
    ...
    if (req.method == "POST") {
      ...
      if (req.path == "/user_comments") {
        ...
        return httpResponse.SetContent(response);
          ...
          return httpResponse.SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
      } else {
          return httpResponse.SetCode(HttpCode::NotFound);
      }
    }
