...
//        os << key_val.fist << ": " << key_val.second << '\n';
//    }
//    return os;
//}
...
public:
  ...
  WriteAccess operator[](const K& key){
      LockedMap& lm = submaps[GetBin(key)];
      return {lock_guard(lm.mt), lm.Map[key]};
  }
...
private:
    size_t GetBin(const K &key) const { return Hash()(key) % submaps.size(); };
    vector<LockedMap> submaps;
};
...
bool ConcurrentMap<K, V, Hash>::Has(const K &key) const {
    ...
    {
        const LockedMap &submap = submaps.at(bin);
        lock_guard<mutex> lock (submap.mt);
        return submap.Map.find(key) != submap.Map.end();
    }
}
...
ConcurrentMap<K, V, Hash>::BuildOrdinaryMap() const {
    MapType aggr;
    for (const LockedMap& submap: submaps){
        lock_guard<mutex> lock(submap.mt);
        aggr.insert(submap.Map.begin(), submap.Map.end());
    }
    return aggr;
}

template<typename K, typename V, typename Hash>
typename ConcurrentMap<K, V, Hash>::ReadAccess
ConcurrentMap<K, V, Hash>::At(const K &key) const {
    if (!Has(key))
        throw out_of_range("Key not found");
    size_t bin = GetBin(key);
    const LockedMap& lm = submaps[bin];
    return {lock_guard<mutex>(lm.mt), lm.Map.at(key)};
}
